...
tasks.set("clean", () =>
  Promise.resolve().then(() =>
    del(["coverage/*", "wwwroot/dist", "server/bin/*"], { dot: true })
  )
);
  ...
  () =>
    new Promise((resolve, reject) => {
      if (!fs.existsSync("wwwroot/dist")) {
        ...
        console.log(
          'Nothing to do. If you want to regenerate "vendor" assets please delete "wwwroot/dist" folder or run "npm run clean" command'
        );
        resolve();
      }
    })
    ...
    new Promise(resolve => {
      ...
      environments.forEach(env => {
        const filename = path.resolve(
          __dirname,
          `./server/appsettings.${env}.json`
        );
        try {
          fs.writeFileSync(filename, JSON.stringify(source, null, "  "), {
            flag: "wx"
          });
        } catch (err) {}
        if (--count === 0) resolve();
        ...
        new Promise(resolve => {
          ...
          compiler.hooks.done.tap("done", () => {
            ...
            if (++count === 1) {
              ...
              cp.spawn("dotnet", ["watch", "run"], options).stdout.on(
                "data",
                data => {
